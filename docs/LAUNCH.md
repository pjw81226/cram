...
- [ ] **Publish to npm** so `npx cram-cli` works: `npm publish` (runs the build via `prepublishOnly`). Do a dry run first: `npm publish --dry-run`.
- [ ] **Verify the one‑liner from a clean shell:** `npx cram-cli@latest --help`.
...
It's Node + Ink, runs with `npx cram-cli`, tokenizes locally (no API key,
...
- npx cram-cli — no install, no API key, tokenizes locally
...
- npx cram-cli — 설치·API 키 불필요, 로컬 토크나이즈, 오프라인 동작
